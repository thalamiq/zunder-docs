...
Ferrum supports **FHIR Bundles as multi-request envelopes** when you `POST` a
...
This page documents **Ferrum-specific behavior**, especially the transaction mechanics.
...
<Note>
  In `Bundle.entry.request.url`, Ferrum accepts both relative and absolute URLs (it strips scheme/host
...
Ferrum supports these response styles for **both batch and transaction**:
...
### Independence rules Ferrum enforces

Ferrum pre-validates and flags non-conformant patterns per entry:
...
Transactions are **atomic**: Ferrum runs the entire bundle inside a single database transaction.
...
Ferrum processes entries in this order (regardless of original order):
...
- `GET`/`HEAD` inside a transaction is instance-read only; if the URL has no id (looks like a
  type-level search), Ferrum returns an empty `searchset` bundle entry.
...
Ferrum implements this by building a mapping and rewriting resources **before writing them**:

1. For non-`POST` entries, if `entry.fullUrl` exists and the request URL is an identity
   (`{type}/{id}`), Ferrum maps `fullUrl → "{type}/{id}"`.
2. For `POST` entries, Ferrum **pre-reserves UUID ids** and maps `fullUrl → "{type}/{uuid}"` before
   processing entries, so later entries can safely reference the would-be created resources.
3. Before `POST`/`PUT`/`PATCH`, Ferrum rewrites string values inside the resource JSON:
...
Ferrum supports **conditional references** (FHIR “search URIs”) inside request resources:
...
- Ferrum searches the target type (`Patient` in the example) using the query string parameters.
...
- If there is **exactly 1 match**, Ferrum replaces the search URI with a normal reference
  (`Patient/{id}`).
...
Ferrum supports the common conditional patterns and resolves them *within the transaction*, meaning:
...
Ferrum enforces version checks where applicable:
...
For transaction `PATCH`, Ferrum expects the entry’s `resource` to be a `Binary` containing a JSON Patch
document:

- `Binary.contentType = "application/json-patch+json"`
- `Binary.data` = base64-encoded JSON Patch bytes

Security/safety note: after applying the patch, Ferrum removes `resource.text` (narrative) because it
...
Ferrum implements the FHIR extension:
...
(`{type}/{id}`), Ferrum will (after all writes) rewrite it to a version-specific reference:
...
Ferrum will rewrite `subject.reference` to `Patient/{uuid}` before inserting the Observation.
